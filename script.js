const DATA = {
    batsmen: [
        ...
        { name: "Kumar Sangakkara", stats: { Power: 82, Technique: 95, Aggression: 80, Composure: 92, Experience: 94 }},
...
let state = {
    ...
    playerRole: 'batting', playerHand: [], aiHand: [], battleStat: null, aiSelectedCardIndex: -1, playerSelectedCardIndex: -1
};
...
window.startRound = function() {
    ...
    if (state.round === 1) {
        ...
        state.playerHand = getRandomCards(DATA.batsmen, 5);
        state.aiHand = getRandomCards(DATA.bowlers, 5);
    } else if (state.round === 2) {
        state.playerRole = 'bowling';
        state.playerHand = getRandomCards(DATA.bowlers, 5);
        state.aiHand = getRandomCards(DATA.batsmen, 5);
    } else {
        state.playerRole = 'allrounder';
        state.playerHand = getRandomCards(DATA.allrounders, 3);
        ...
        <div class="screen">
            <div class="full-center" style="animation: slideUp 0.5s ease">
                ...
                <div style="background: var(--card-bg); padding: 15px; border-radius: 12px; margin-bottom: 30px; width: 85%; text-align: left; border: 1px solid var(--card-border)">
                    <div style="font-weight: 700; margin-bottom: 10px; color: var(--accent)">YOUR HAND:</div>
